...
class HeadSpace2_Plugin
{
	...
	/**
	 * Register that you want plugin version information to be included in the plugin management page
	 *
	 * @param string $version_url URL of a file containing version information
	 * @return void
	 **/
	
	function register_plugin_details ($version_url)
	{
		$this->version_url = $version_url;
		add_action ('after_plugin_row', array (&$this, 'plugin_version'), 11, 3);
	}
	
	
	/**
	 * Hook that retrieves the plugin version information file and displays only the information that is newer than
	 * the current plugin version.  File format is:
	 *   version
	 *   details (HTML)
	 *   ==== (four equal signs)
	 *
	 * For example:
	 * 0.5.7
	 * <p>New version with fancy functions</p>
	 * 0.5.6
	 * <p>Older version with something else</p>
	 *
	 * @return void
	 **/
	
	function plugin_version ($file, $details = '', $context = '')
	{
		if ($details != '' && basename ($this->plugin_base) == dirname ($file) && $this->version_url != null)
		{
			$contents = wp_remote_fopen ($this->version_url);
			...
			if (strlen ($contents) > 0)
			{
				...
				if (count ($parts) > 0)
				{
					foreach ($parts AS $part)
					{
						...
						$html    = trim (implode ('', $lines));
						if (version_compare ($version, $details['Version']) == 1)
						{
?>
	<tr>
		<td colspan="2" class="plugin-update"/>
		<td class="plugin-update" style="line-height:1.2em;"><?php echo htmlspecialchars ($version); ?></td>
		<td colspan="2" class="plugin-update" style="line-height:1.2em; text-align: left"><?php echo $html; ?></td>
	</tr>
<?php
						}
					}
...
if (!class_exists ('HS_Plugin'))
{
	class HS_Plugin
	{
		function HS_Plugin ($name, $max = 1, $id = '', $args = '')
		{
			$this->name        = $name;
			$this->id          = $id;
			$this->widget_max  = $max;
