    ...
    if ($results) {
        if($return_bool) {
			...
			foreach ($results as $result) {
				...
				$output .= $before_title .'<a href="'. $permalink .'" rel="bookmark" title="'. $title .'">' . $title . '</a>' . $after_title;
...
//					$output .= $before_post . $post_strip . $after_post;
        ...
        if($return_bool) {
			...
			echo $before_title.'No hay art&iacute;culos relacionados a&uacute;n.'.$after_title;
		}
    }
}
